...
В **00:00 по часовому поясу пользователя** главное сообщение автоматически
обновляется на новый день: меняются дата, записи и кнопка таблетки.
Бот возвращается в обычный режим ввода за сегодня. Обновление работает
и при выключенных напоминаниях; для него бот должен быть запущен.

## Напоминания

Ежедневно бот присылает вопрос о головной боли и отдельное сообщение
«выпил таблетку ?» с единственной кнопкой «Да». В главном меню вместо
статистики есть кнопка «💊 Таблетки». Она сразу отмечает приём за текущий
день и меняется на «✅ Выпил». Нажатие «Да» в напоминании делает то же
самое и убирает напоминание. В обоих случаях появляется отдельное
подтверждение без кнопок, которое удаляется через 5 секунд.
Отметка сохраняется после перезапуска; при обновлении меню в следующий
день (в часовом поясе пользователя) снова отображается «💊 Таблетки».
Дополнительного опроса и записи в дневник нет.
